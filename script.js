(function() {
    ...
    styleTag.textContent = `
        ...
        .ban-all-btn { background: rgba(231,76,60,0.1); color: #e74c3c; margin-top: 8px; display: inline-block; }
    `;
    ...
    async function renderBannedList() {
        ...
        if (!data || data.length === 0) {
            container.innerHTML += '<p style="color:var(--text-muted);">Нет</p>';
            return;
        }
        data.forEach(entry => {
            ...
            unblock.style.cssText = 'background:#2ecc71;border:none;color:white;padding:2px 8px;border-radius:4px;cursor:pointer;font-size:0.75rem;';
    ...
    // АНТИСПАМ: проверка на повторяющиеся символы
    function isSpamText(text) {
        ...
        // Если один символ занимает более 70% текста или текст состоит из менее чем 3 уникальных символов и длина > 10
        ...
        // Проверка на повторяющиеся короткие паттерны (например, "бе" повторяется много раз)
        const pattern = detectRepeatingPattern(text);
        if (pattern && text.length > 10) return true;
        return false;
    }

    function detectRepeatingPattern(text) {
        // Ищем повторяющийся паттерн длиной от 1 до 4 символов
    ...
    function canPublish() {
        ...
        const now = Date.now();
        const cooldown = 5000; // 5 секунд между постами
    ...
    function updatePublishButtonState() {
        ...
        else if (!canPost && hasContent) showComposerError('Слишком часто! Подождите 5 секунд.');
    }
    ...
    async function publishPost() {
        ...
        // Проверка на спам-текст
        if (content && isSpamText(content)) {
            showComposerError('Сообщение отклонено: обнаружен спам. Измените текст.');
            return;
        }
        
        // Проверка rate limit
        if (!canPublish()) {
            showComposerError('Слишком часто! Подождите 5 секунд.');
            return;
        }
    ...
    // Автоматическое обновление статуса кнопки при вводе (для rate limit)
    ...
    async function init() {
        ...
        // Периодическая проверка, не истек ли rate limit
