const NobuWave = (() => {
    ...
    const openChat = async (chatId) => {
        ...
        document.getElementById('imageInput').addEventListener('change', (e) => { const file = e.target.files[0]; if (!file) return; pendingImage = file; pendingVoice = null; updatePendingMedia(); });
        document.getElementById('voiceRecordBtn').addEventListener('click', async () => {
            if (isRecording) { mediaRecorder.stop(); isRecording = false; document.getElementById('voiceRecordBtn').classList.remove('recording'); document.getElementById('voiceRecordBtn').innerHTML = '<i class="fa-solid fa-microphone"></i>'; return; }
            try {
                ...
                mediaRecorder.onstop = () => {
                    ...
                    pendingVoice = { blob, duration: durStr };
                    pendingImage = null;
                    updatePendingMedia();
        ...
        document.getElementById('clearPendingMedia').addEventListener('click', () => { pendingImage = null; pendingVoice = null; updatePendingMedia(); });
    ...
    const updatePendingMedia = () => {
        ...
        if (pendingImage) {
            ...
            content.innerHTML = `<img src="${URL.createObjectURL(pendingImage)}" style="max-height:60px;border-radius:8px"><span style="color:var(--text-secondary);font-size:0.8rem">Фото</span>`;
        } else if (pendingVoice) {
            container.style.display = 'block';
            content.innerHTML = `<span class="pending-media-voice">🎤 Голосовое (${pendingVoice.duration})</span>`;
        } else {
    ...
    const loadMessages = async (chatId) => {
        ...
        list.innerHTML = data?.map(msg => {
            ...
            if (msg.image_url) html += `<img src="${esc(msg.image_url)}" onclick="window.open('${esc(msg.image_url)}')">`;
            if (msg.video_url) html += `<video src="${esc(msg.video_url)}" controls></video>`;
        ...
        list.querySelectorAll('.voice-play-btn').forEach(btn => { let audio = null; btn.addEventListener('click', () => { const url = btn.dataset.url; if (audio && !audio.paused) { audio.pause(); btn.innerHTML = '<i class="fa-solid fa-play"></i>'; return; } if (audio) audio.pause(); audio = new Audio(url); btn.innerHTML = '<i class="fa-solid fa-pause"></i>'; audio.play(); audio.onended = () => { btn.innerHTML = '<i class="fa-solid fa-play"></i>'; }; }); });
    ...
    const sendMessage = async () => {
        ...
        if (!content && !pendingImage && !pendingVoice) return;
        try {
            let imageUrl = null, voiceUrl = null, voiceDur = null;
            if (pendingImage) { const path = `chats/${currentUser.id}_${Date.now()}.${pendingImage.name.split('.').pop()}`; await supabase.storage.from('images').upload(path, pendingImage); const { data } = supabase.storage.from('images').getPublicUrl(path); imageUrl = data.publicUrl; }
            if (pendingVoice) { const path = `voices/${currentUser.id}_${Date.now()}.webm`; await supabase.storage.from('images').upload(path, pendingVoice.blob); const { data } = supabase.storage.from('images').getPublicUrl(path); voiceUrl = data.publicUrl; voiceDur = pendingVoice.duration; }
            const { error } = await supabase.from('messages').insert({ chat_id: activeChat.id, user_id: currentUser.id, username: currentUser.username, unique_id: currentUser.unique_id, content: content || '', image_url: imageUrl, voice_url: voiceUrl, voice_duration: voiceDur, is_verified: currentUser.is_verified || false });
            if (error) throw error;
            input.value = ''; pendingImage = null; pendingVoice = null; updatePendingMedia(); input.focus();
    ...
    const showSupportModal = () => {
        const overlay = document.createElement('div'); overlay.className = 'modal-overlay';
        overlay.innerHTML = `<div class="modal-card"><h3>🎧 Поддержка</h3><p style="color:var(--text-secondary);text-align:center;margin-bottom:12px">Опишите проблему. Ответим в течение 5 рабочих дней.</p><textarea id="supportQuestion" class="modal-input" placeholder="Опишите проблему..." style="height:120px;resize:none"></textarea><button class="modal-btn" id="sendSupportBtn">Отправить</button><button class="modal-btn secondary" id="closeSupportBtn">Закрыть</button></div>`;
        document.body.appendChild(overlay); document.getElementById('closeSupportBtn').addEventListener('click', () => overlay.remove()); document.getElementById('sendSupportBtn').addEventListener('click', async () => { const q = document.getElementById('supportQuestion').value.trim(); if(!q)return; await supabase.from('support').insert({user_id:currentUser.id,username:currentUser.username,question:q}); alert('✅ Отправлено! Ответим в течение 5 рабочих дней.'); overlay.remove(); });
    };
    ...
    const showAdminPanel = () => {
        ...
        const ls = async () => { const { data } = await supabase.from('support').select('*').order('created_at', { ascending: false }).limit(20); document.getElementById('supportList').innerHTML = data?.length ? data.map(s => `<div style="padding:6px 0;border-bottom:1px solid var(--border);font-size:0.8rem"><strong>${esc(s.username)}</strong>: ${esc(s.question)}<br><small>${new Date(s.created_at).toLocaleString('ru-RU')}</small>${!s.is_answered?`<br><button class="modal-btn" style="margin-top:4px;padding:4px 8px;font-size:0.75rem" id="answer_${s.id}">✉️ Ответить</button>`:'<br><small style="color:var(--success)">✓ Отвечено</small>'}</div>`).join('') : '<p style="color:var(--text-secondary);font-size:0.8rem">Нет</p>'; setTimeout(() => { document.querySelectorAll('[id^="answer_"]').forEach(btn => { btn.addEventListener('click', async () => { const sid = btn.id.replace('answer_',''); const { data: support } = await supabase.from('support').select('*').eq('id', sid).single(); if (!support) return; const { data: user } = await supabase.from('users').select('*').eq('username', support.username).single(); if (!user) return; const chatName = [currentUser.username, user.username].sort().join(' & '); const { data: existChat } = await supabase.from('chats').select('*').eq('name', chatName).eq('is_group', false).single(); if (existChat) { await supabase.from('support').update({ is_answered: true, answered_by: currentUser.id }).eq('id', sid); document.querySelector('.modal-overlay')?.remove(); openChat(existChat.id); return; } const { data: chat } = await supabase.from('chats').insert({ name: chatName }).select().single(); await supabase.from('chat_members').insert([{ chat_id: chat.id, user_id: currentUser.id }, { chat_id: chat.id, user_id: user.id }]); await supabase.from('support').update({ is_answered: true, answered_by: currentUser.id }).eq('id', sid); document.querySelector('.modal-overlay')?.remove(); openChat(chat.id); }); }); }, 100); };
        lb(); lr(); ls();
