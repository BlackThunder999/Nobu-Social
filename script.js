(function() {
    ...
    // Динамические стили (под новую тему)
    ...
    styleTag.textContent = `
        ...
        .ban-all-btn { background: rgba(255,255,255,0.05); color: #ffffff; margin-top: 8px; display: block; width: 100%; text-align: center; padding: 8px; border-radius: 6px; }
        .avatar-upload-area {
            display: flex;
            align-items: center;
            gap: 12px;
        ...
        .avatar-upload-btn:hover { background: #252525; color: #ffffff; }
        .unblock-btn {
            background: #888888;
            border: none;
            color: #0a0a0a;
        ...
        .unblock-btn:hover { background: #aaaaaa; }
    `;
    ...
    async function renderBannedList() {
        ...
        if (!container) return;
        container.innerHTML = '<h4 style="margin-bottom:8px;color:#ffffff;">🚫 Заблокированные</h4>';
        const { data } = await supabase.from('banned_users').select('*');
        if (!data || data.length === 0) {
            container.innerHTML += '<p style="color:#666666;">Нет заблокированных</p>';
            return;
        }
        data.forEach(entry => {
            const div = document.createElement('div');
            div.style.cssText = 'display:flex;justify-content:space-between;align-items:center;padding:6px 0;color:#ffffff;border-bottom:1px solid rgba(255,255,255,0.1);';
    ...
    function handleSaveNickname() {
        ...
        if (!newNick) {
            nicknameInput.style.border = '1px solid #555555';
            nicknameInput.focus();
