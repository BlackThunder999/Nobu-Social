(function() {
    ...
    async function toggleFollow(userId, btn) {
        ...
        if (viewingUserId === userId) {
            ...
            document.getElementById('statFollowers').innerHTML = `<span class="profile-stat-value">${f}</span> подписчиков`;
            document.getElementById('statFollowing').innerHTML = `<span class="profile-stat-value">${fg}</span> подписок`;
        }
    }
    ...
    function createPostCard(post) {
        ...
        let media = '';
        if (post.image_url) media += `<div class="post-card-img"><img src="${esc(post.image_url)}"></div>`;
        if (post.video_url) media += `<div class="post-card-video"><video controls src="${esc(post.video_url)}"></video></div>`;
        card.innerHTML = `
    ...
    function updatePublishBtn() {
        DOM.publishBtn.disabled = (!DOM.postTextarea.value.trim() && !selectedImage && !selectedVideo) || isPublishing || bannedUserIds.has(currentUser?.id);
    }
