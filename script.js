(function() {
    ...
    const VERIFY_PASSWORD = 'NobuSocialAdmin2024';
    ...
    function setVerifiedStatus(status) {
        localStorage.setItem(STORAGE_VERIFIED_KEY, status);
    }

    function updateVerifiedBadge() {
        // Обновляем значок в композере
        const existingBadge = composerNickname.querySelector('.verified-badge');
        if (isVerified) {
            if (!existingBadge) {
    ...
    // При сохранении ника проверяем пароль, если ник NobuSocial
    function handleSaveNickname() {
        ...
        if (newNick === 'NobuSocial') {
            // Запрашиваем пароль верификации
            const password = prompt('Введите пароль для верификации аккаунта NobuSocial:');
            if (password === VERIFY_PASSWORD) {
                ...
                if (password !== null) {
                    alert('Неверный пароль! Ник будет сохранён без верификации.');
                }
            }
        } else {
            // Если ник другой, сбрасываем верификацию
            isVerified = false;
            setVerifiedStatus(false);
        }
