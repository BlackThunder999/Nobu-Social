const NobuGram = (() => {
    ...
    const renderApp = () => {
        app.innerHTML = `
            <div class="app-container">
                <div class="header">
                    <div class="header-title">💬 NobuGram</div>
    ...
    const loadChats = async () => {
        ...
        if (chatIds.length === 0) {
            document.getElementById('chatList').innerHTML = '<div style="text-align:center;padding:40px;color:var(--text-secondary)">Нет чатов. Создайте новый!</div>';
            return;
        }
    ...
    const showProfileModal = () => {
        ...
        overlay.innerHTML = `
            <div class="modal-card">
                <h3>Профиль</h3>
                <p style="font-size:1.2rem;font-weight:600">${currentUser.username}</p>
                <p style="color:var(--text-secondary);margin:10px 0">Выберите эмодзи:</p>
                <div style="display:flex;flex-wrap:wrap;gap:8px">${['👤','😀','😎','🤖','👽','🦊','🐼','🎃','💎','🔥'].map(e => `<span style="font-size:2rem;cursor:pointer" class="emoji-opt">${e}</span>`).join('')}</div>
                <button class="modal-btn secondary" id="logoutBtn" style="margin-top:10px">Выйти</button>
    ...
    const setupNavigation = () => {
        document.querySelectorAll('.tab').forEach(tab => {
            tab.addEventListener('click', () => {
                ...
                // Здесь можно переключать список чатов / письма
            });
        });
    };
...
document.addEventListener('DOMContentLoaded', () => NobuGram.init());